...
class ReservationApi extends Utils {
  ...
  Future<bool> addReservation(Future<int> userId, int idBook) async {
    ...
    var response = await http.post('$urlServer/prenotazioni/addPrenotazione',
        headers: header, body: reservation.bookedJson());
  ...
  Future<bool> returnBook(int idReservation) async {
    ...
    var response = await http.put('$urlServer/prenotazioni/returnBook',
        headers: header, body: reservation.returnedJson());
  ...
  Future<List<Reservation>> getBooksToBeReturned(Future<int> userId) async {
    ...
    var response = await http
        .get('$urlServer/prenotazioni/daRestituire/$idUser', headers: header);
    ...
    for (var el in jsonDecode(response.body)['data'][0]) {
      ...
      list.add(Reservation(
          ...
          dateReservation: DateTime.parse(
              el['DataPrenotazione'].toString().substring(0, 10))));
    }
